(function () {
  ...
  function cp1251Byte(code) {
    ...
    if (Object.prototype.hasOwnProperty.call(CP1251_EXTRA_BYTES, code)) return CP1251_EXTRA_BYTES[code];
  ...
  function fixMojibake(root) {
    ...
    textNodes.forEach((node) => {
      const decoded = decodeMojibakeText(node.nodeValue);
    ...
    ['placeholder', 'title', 'alt', 'aria-label', 'value'].forEach((attr) => {
      scope.querySelectorAll?.(`[${attr}]`).forEach((node) => {
        ...
        const decoded = decodeMojibakeText(current);
  ...
  function showToast(text) {
    ...
    toast.textContent = decodeMojibakeText(text);
  ...
  function init() {
    ...
    document.addEventListener('change', (event) => {
      if (event.target.matches('select[name="attribute_pa_color"], input.variation_id')) {
        ...
        window.setTimeout(() => fixMojibake(document), 0);
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('.thwvsf-wrapper-item-li, .thwvsf-selected, .rtwpvg-thumbnail, [data-value]')) {
        ...
        window.setTimeout(() => fixMojibake(document), 80);
